...
def pytest_generate_tests(metafunc):
    files = [os.path.join(TESTDATA_DIR, name) for name in
             os.listdir(TESTDATA_DIR) if name.endswith('fits')]
...
def test_main_app(test_name):
    ...
    with patch('caom2utils.fits2caom2.CadcDataClient') as data_client_mock:
        ...
        # sys.argv = \
        #     ('vlass2caom2 --no_validate --ignorePartialWCS --local {} '
        #      '--plugin {} --observation VLASS {} -o {} --lineage {}'.
        #     format(local, plugin, product_id, output_file,
        #            lineage)).split()
        sys.argv = \
            ('vlass2caom2 --ignorePartialWCS --local {} '
             '--observation VLASS {} -o {} --lineage {}'.
             format(local, product_id, output_file,
                    lineage)).split()
        print(sys.argv)
        main_app()
        obs_path = '{}.xml'.format(test_name)
